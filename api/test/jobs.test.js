...
async function settle(uid, ms = 15000) {
  ...
  while (Date.now() < until) {
    const s = await jobs.status(uid);
    if (!s.job) return s;
    await new Promise(r => setTimeout(r, 25));
  }
  throw new Error('job never finished');
}
async function lastOutcome(uid) { return (await jobs.readUser(uid)).history.at(-1); }
...
test('a review job produces a validated, applyable proposal', async () => {
  ...
  assert.equal((await lastOutcome(uid)).outcome, 'ready');
...
test('a creation job produces a plan bundle in the app\'s own share format', async () => {
  ...
  await jobs.enqueue(uid, { kind: 'create', intake: { goal: 'muscle', daysPerWeek: 3, equipment: ['dumbbell'] } });
  const s = await settle(uid);
  assert.equal((await lastOutcome(uid)).outcome, 'ready');
...
test('one job per profile at a time', async () => {
  const uid = 'u-single';
  writeState(DIR, uid, sampleState());
  const first = jobs.enqueue(uid, { kind: 'review' });
...
test('the daily cap is enforced and reported as its own failure', async () => {
  ...
  assert.equal((await jobs.capState(uid)).used, 1);
...
test('a malformed answer is repaired once, then accepted', async () => {
  ...
  assert.equal((await lastOutcome(uid)).outcome, 'ready', 'the repair round rescued it');
  assert.ok(s.pending.changes.length > 0);
});
...
test('an answer that stays unusable fails cleanly and applies nothing', async () => {
  ...
  const s = await settle(uid);
  delete process.env.FIXTURE_MODE;
  const last = await lastOutcome(uid);
...
test('a provider that crashes is reported, not retried behind the user\'s back', async () => {
  ...
  await settle(uid);
  delete process.env.FIXTURE_MODE;
  assert.equal((await lastOutcome(uid)).outcome, 'failed');
  assert.equal((await jobs.readUser(uid)).history.filter(h => h.outcome === 'failed').length, 1, 'exactly one attempt');
});
...
test('nothing worth changing produces no proposal and nothing to notify about', async () => {
  ...
  const s = await settle(uid);
  delete process.env.FIXTURE_MODE;
  assert.equal((await lastOutcome(uid)).outcome, 'nochange');
...
test('resolving a proposal records the decision and clears it', async () => {
  ...
  await jobs.enqueue(uid, { kind: 'review' });
  const s = await settle(uid);
  await jobs.resolvePending(uid, { accepted: [s.pending.changes[0].id], rejected: [] });
  assert.equal((await jobs.status(uid)).pending, null);
  assert.equal((await lastOutcome(uid)).outcome, 'applied');
  assert.equal((await lastOutcome(uid)).accepted, 1);
});
...
test('forgetting a profile leaves no server-side residue', async () => {
  ...
  await jobs.enqueue(uid, { kind: 'review' });
  await settle(uid);
  await jobs.clearUser(uid);
  const s = await jobs.status(uid);
  assert.equal(s.pending, null);
  assert.deepEqual((await jobs.readUser(uid)).history, []);
});
...
test('a job interrupted by a restart is reported as failed, not left spinning', async () => {
  ...
  await jobs.recoverOnBoot();
  assert.equal((await jobs.status(uid)).job, null);
  assert.equal((await lastOutcome(uid)).errorClass, 'restart');
});
