...
test('payload never carries identity, credentials or device data', () => {
  ...
  const p = payload.build(S, 'user-abc-123', SECRET, { kind: 'review' });
...
test('the same profile always gets the same handle, and two profiles never share one', () => {
  const S = sampleState();
  const a1 = payload.build(S, 'uid-a', SECRET, { kind: 'review' }).meta.profile;
  const a2 = payload.build(S, 'uid-a', SECRET, { kind: 'review' }).meta.profile;
  const b = payload.build(S, 'uid-b', SECRET, { kind: 'review' }).meta.profile;
  assert.equal(a1, a2);
  assert.notEqual(a1, b);
});

test('review payload carries the plan, the window, effort and aggregates', () => {
  const p = payload.build(sampleState(), 'u1', SECRET, { kind: 'review', note: 'shoulder pinches' });
...
test('a stalling exercise shows up in the aggregates the way the engine counts it', () => {
  ...
  const p = payload.build(S, 'u1', SECRET, { kind: 'review' });
...
test('creation payload carries working weights so baselines start from evidence', () => {
  const p = payload.build(sampleState(), 'u1', SECRET, { kind: 'create' });
