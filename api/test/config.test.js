...
test('a credential encrypted under a different secret fails closed', async () => {
...
test('retired Gemini and Custom command configurations reset to unconfigured Claude', async () => {
  cfg.save({
    enabled: true,
    provider: 'gemini',
...
/* ---------------- device-login output redaction ---------------- */

test('device-login output keeps the URL and verification code an admin needs', () => {
  const out = auth.cleanLoginOutput('Open https://chatgpt.com/device and enter code ABCD-1234');
  assert.equal(out, 'Open https://chatgpt.com/device and enter code ABCD-1234');
});

test('device-login output redacts a JWT-shaped value', () => {
  const jwt = 'eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiIxMjM0NTY3ODkwIn0.SflKxwRJSMeKKF2QT4fwpMeJf36POk6yJV_adQssw5c';
  const out = auth.cleanLoginOutput(`session token: ${jwt}\ncontinue in the browser`);
  assert.ok(!out.includes(jwt), 'the raw token never reaches the admin UI');
  assert.ok(out.includes('[redacted]'));
  assert.ok(out.includes('continue in the browser'), 'surrounding instructions survive redaction');
});

test('device-login output strips ANSI escape sequences', () => {
  const out = auth.cleanLoginOutput('\x1b[32mWaiting for sign-in\x1b[0m...');
  assert.equal(out, 'Waiting for sign-in...');
});

test('device-login output does not redact short dotted values below the JWT threshold', () => {
  const out = auth.cleanLoginOutput('code: ABCD.1234.efgh');
  assert.equal(out, 'code: ABCD.1234.efgh');
});

test('device-login output is capped to the last 4096 characters', () => {
  const out = auth.cleanLoginOutput('a'.repeat(5000) + 'END');
  assert.equal(out.length, 4096);
  assert.ok(out.endsWith('END'));
});

test('device-login output handles empty and non-string input', () => {
  assert.equal(auth.cleanLoginOutput(''), '');
  assert.equal(auth.cleanLoginOutput(null), '');
  assert.equal(auth.cleanLoginOutput(undefined), '');
});
