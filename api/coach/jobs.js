 ...
 * here, in a record only this server writes, until the user decides what to do with it — at
 ...
 *
 * Everything here is deliberately a single in-process queue, file- or Postgres-backed via
 * api/store/. A single-core box running one household's gym app — or a SaaS instance behind
 * one API process — does not need a job queue with a broker; it needs jobs that cannot pile
...
export async function readUser(uid) {
  const rec = await store.readCoachUser(uid);
  return { ...EMPTY, ...(rec || {}) };
}
async function writeUser(uid, rec) { await store.writeCoachUser(uid, rec); }
async function patchUser(uid, patch) {
  const rec = { ...(await readUser(uid)), ...patch };
  await writeUser(uid, rec);
  return rec;
}
/** Consent revoked, profile deleted, "reset everything" — no server-side residue (FR-51). */
export async function clearUser(uid) { await store.deleteCoachUser(uid); }

export async function readState(uid) { return store.readState(uid); }

/* ---------- caps ---------- */

const todayISO = () => new Date().toISOString().slice(0, 10);
async function bumpDaily(uid) {
  const rec = await readUser(uid);
  const d = todayISO();
  const daily = rec.daily?.date === d ? { date: d, count: rec.daily.count + 1 } : { date: d, count: 1 };
  await patchUser(uid, { daily });
  return daily.count;
}
export async function capState(uid) {
...
async function archive(uid, rec, outcome) {
  const history = [...(rec.history || []), {
    id: rec.pending?.id, kind: rec.pending?.kind, outcome, at: Date.now()
  }].slice(-HISTORY_MAX);
  await patchUser(uid, { pending: null, history });
}
...
/**
 * Enqueue a job. Throws (rejects with) CoachError with a code the routes layer maps to an HTTP
 * status: `off`, `busy`, `cap`, `consent`.
 */
export async function enqueue(uid, opts) {
  ...
  // Marked in-flight synchronously, in the same tick as the check above: every await from here
  // down is a point where a second call for the same uid could otherwise interleave and slip
  // past the `busy` check before this one finishes claiming it (FR-07 single-flight).
  inflight.add(uid);
  try {
    ...
    await patchUser(uid, { current: { id: job.id, kind: job.kind, state: 'queued', startedAt: job.startedAt } });
    queue.push(job);
    pump();
    return { id: job.id };
  } catch (e) {
    // Nothing was queued — pump()'s own `.finally()` is what clears `inflight` once a job has
    // actually started, so a rejection here has to clear it itself or the uid stays "busy"
    // forever on a consent/cap failure.
    inflight.delete(uid);
    throw e;
  }
}
...
async function execute(job) {
  await patchUser(job.uid, { current: { id: job.id, kind: job.kind, state: 'running', startedAt: job.startedAt } });
  ...
  const pendingCreate = job.refine ? (await readUser(job.uid)).pending : null;
  const payload = payloadLib.build(S, job.uid, secret, {
    kind: job.kind,
    intake: job.intake,
    note: job.note,
    refine: job.refine,
  ...
  try {
    ...
    if (!attempt.ok) {
      return await finish(job, { outcome: 'failed', errorClass: attempt.errorClass, detail: attempt.detail });
    }
    if (attempt.nochange) {
      return await finish(job, { outcome: 'nochange', pending: null, detail: null });
    }
    const pending = {
      id: job.id,
      kind: job.kind,
    ...
    return await finish(job, { outcome: 'ready', pending });
  } finally {
...
export async function resolvePending(uid, { accepted = [], rejected = [], dismissed = false } = {}) {
  const rec = await readUser(uid);
  ...
  await writeUser(uid, { ...rec, pending: null, history });
  return { ok: true };
}
...
export async function recoverOnBoot() {
  let n = 0;
  const ids = await store.listCoachUserIds();
