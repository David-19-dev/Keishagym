/* opengym-api — passkey (WebAuthn) auth + per-user state storage for openGym
   No framework. Storage is file-based (self-hosted) or Postgres (SaaS, via DATABASE_URL) —
   see api/store/. Sessions are signed cookies either way.                */
...
const RP_NAME = process.env.RP_NAME || 'KeishaGym';
...
/* ---------- secret + db ----------
 * Read once at boot from the store (a file self-hosted, a row in Postgres on the SaaS) and
 * kept in memory from here on — db is mutated in place and persisted with saveDb(), exactly
 * the read-once/write-through pattern the file backend always used, just moved behind the
 * store interface so either backend works underneath it unmodified. */
 ...
 * VAPID keys stay a local file even on the SaaS deployment: they're generated once per
 * instance and need a real filesystem the process can persist across restarts, same as
 * Codex's credential cache in api/coach/config.js — neither is part of the store abstraction. */
...
const routes = {
  ...
  'POST /api/push/test': async (req, res) => {
    const user = readSession(req);
    if (!user) return json(res, 401, { error: 'not signed in' });
    await sendPush(user.id, { title: 'KeishaGym', body: 'Test notification ✅ — this is what alerts look like.', tag: 'test' });
    json(res, 200, { ok: true });
  },
