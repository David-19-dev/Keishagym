 ...
 */
import LIBRARY_JSON from './library.json' with { type: 'json' };

const LIBRARY = LIBRARY_JSON.exercises;
...
/* The stable per-profile pseudonym is computed by the caller (index.ts), because Web Crypto's
   HMAC is async and this builder is not: build() takes the finished handle. Never the uid,
   never reversible, same across jobs. */
 ...
 * @param {string} secret the instance secret `handle()` derives the pseudonym from
 * @param {object} opts   { kind, intake?, note?, refine?, previous? }
 */
export function build(S, handle, opts = {}) {
  const coach = S.coach || {};
